...
class MSSQLQueryGenerator extends AbstractQueryGenerator {
  ...
  createTableQuery(tableName, attributes, options) {
    const query =
        "IF OBJECT_ID('<%= table %>', 'U') IS NULL CREATE TABLE <%= table %> (<%= attributes %>)",
      primaryKeys = [],
      foreignKeys = {},
      attrStr = [];
    ...
    if (options.uniqueKeys) {
      _.each(options.uniqueKeys, (columns, indexName) => {
        if (columns.customIndex) {
          ...
          values.attributes += `, CONSTRAINT ${this.quoteIdentifier(
            indexName,
        ...
        constraintString.push(
          _.template(
            ...
            fkName: this.quoteIdentifier(
              tableName + "_" + attributeName + "_foreign_idx",
            ),
  ...
  renameColumnQuery(tableName, attrBefore, attributes) {
    const query =
        "EXEC sp_rename '<%= tableName %>.<%= before %>', '<%= after %>', 'COLUMN';",
    ...
    if (allAttributes.length > 0) {
      _.forEach(attrValueHashes, attrValueHash => {
        tuples.push(
          "(" +
            allAttributes
              .map(key => this.escape(attrValueHash[key]))
              .join(",") +
            ")",
        );
      });
  ...
  upsertQuery(tableName, insertValues, updateValues, where, model) {
    ...
    for (const index of model.options.indexes) {
      if (index.unique && index.fields) {
        for (const field of index.fields) {
          ...
          if (
            uniqueAttrs.indexOf(fieldName) === -1 &&
    ...
    const insertKeysQuoted = insertKeys
      .map(key => this.quoteIdentifier(key))
      .join(", ");
    const insertValuesEscaped = insertKeys
      .map(key => this.escape(insertValues[key]))
      .join(", ");
  ...
  showConstraintsQuery(tableName) {
    return `EXEC sp_helpconstraint @objname = ${this.escape(
      this.quoteTable(tableName),
    )};`;
  }
  ...
  attributeToSQL(attribute) {
    ...
    if (attribute.references) {
      if (
        attribute.Model &&
        attribute.Model.tableName === attribute.references.model
      ) {
        this.sequelize.log(
  ...
  attributesToSQL(attributes, options) {
    ...
    for (key in attributes) {
      ...
      if (attribute.references) {
        if (
          existingConstraints.indexOf(attribute.references.model.toString()) !==
          -1
        ) {
  ...
  getForeignKeysQuery(table, catalogName) {
    ...
    let sql =
      this._getForeignKeysQueryPrefix(catalogName) +
      " WHERE TB.NAME =" +
      wrapSingleQuote(tableName);
  ...
  rollbackTransactionQuery(transaction) {
    if (transaction.parent) {
      return (
        "ROLLBACK TRANSACTION " + this.quoteIdentifier(transaction.name) + ";"
      );
    }
    ...
    if (
      ...
      if (options.offset) {
        ...
        if (!orders.mainQueryOrder.length) {
          orders.mainQueryOrder.push(
            this.quoteIdentifier(model.primaryKeyField),
          );
        }
        ...
        mainFragment =
          "SELECT " + topFragment + attributes.join(", ") + " FROM " + tables;
      }
    }
  ...
  addLimitAndOffset(options, model) {
    ...
    if (options.limit || options.offset) {
      if (!options.order || (options.include && !orders.subQueryOrder.length)) {
        ...
        fragment +=
          this.quoteTable(options.tableAs || model.name) +
          "." +
          this.quoteIdentifier(model.primaryKeyField);
      }
