      ...
      it("properly generate alter queries for foreign keys", () => {
        return current
          ...
          .then(sql => {
            expectsql(sql, {
              mssql:
                "ALTER TABLE [users] ADD CONSTRAINT [users_level_id_foreign_idx] FOREIGN KEY ([level_id]) REFERENCES [level] ([id]) ON DELETE CASCADE;",
              mysql:
                "ALTER TABLE `users` ADD CONSTRAINT `users_level_id_foreign_idx` FOREIGN KEY (`level_id`) REFERENCES `level` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;",
              postgres:
